...
import h5py
from post_challenge_data_utils import load_rna_features_json

INPUT_PATH = Path("/input")
OUTPUT_PATH = Path("/output")
RESOURCE_PATH = Path("resources")
SURVIVAL_WEIGHTS_PATH = Path("/opt/app/resources/clin2_rna_wsi")
SCALER_WEIGHTS_PATH = Path("/opt/app/resources/clin2_rna_wsi")

def maybe_reduce_rna(val_rna, run=0, fold=0):
    
    pca_path = os.path.join(SURVIVAL_WEIGHTS_PATH, f"rna_pca_run_{run}_{fold}.pkl")
...
def inference():
    ...
    test_clin_array, _ = extract_clinical_vector(jsdata)


    rna_json_path = INPUT_PATH / "bulk-rna-seq-bladder-cancer.json"
    test_rna_array = load_rna_features_json(rna_json_path)  # Shape: (1, num_genes)
    ...
    if USE_ENSEMBLE:
        ...
        for fold_idx in range(NUM_FOLDS):
            ...
            rna_tensor = torch.tensor(fold_rna_array, dtype=torch.float32).to(device) if fold_rna_array is not None else torch.zeros((1, r_dim), device=device)
