...
FUSION_TYPE = 'simple'  # Options: 'modality' (softmax weights per modality) or 'linear' (linear layer after concat) or 'simple' (concat with no learnable params),  'cross_att' (AS cross attention fusion)
...
if SURVIVAL_MODEL == 'cox':
    ...
    GLOBAL_DIR = f"{OUTPUT_DIR}clin_{USE_CLINICAL_FEATURES}_rna_{USE_RNA_FEATURES}_wsi_{USE_WSI_FEATURES}_embed_{EMBEDDER}_model_{SURVIVAL_MODEL}_fuse_{FUSION_TYPE}_loss_{DEEPHIT_LOSS}_scale_{SCALE_DATA}_ep_{EPOCHS}_ModDrop_{DROP_MODALITY}_binEdg_{BIN_EDGES}_ISBI_concat/" ## main path for results
