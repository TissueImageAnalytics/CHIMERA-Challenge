...
#GLOBAL_DIR = f"{OUTPUT_DIR}CLIN_{USE_CLINICAL_FEATURES}_MRI_{USE_MRI_FEATURES}_RAD_{USE_RADIOMIC_FEATURES}_WSI_{USE_WSI_FEATURES}_MAG_{MAG}_PATCH_{PATCH_SIZE}_PATCHES_{PATCHES}_MASK_{MASKER}_EMBED_{EMBEDDER}_MODEL_{SURVIVAL_MODEL}_FUSE_{FUSION_TYPE}_SCALE_{SCALE_DATA}_EP_{EPOCHS}/" ## main path for results
GLOBAL_DIR = f"{OUTPUT_DIR}task1_submit3_clin_rad_wsi_500patches"