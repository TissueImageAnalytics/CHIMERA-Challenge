...
GLOBAL_DIR = f"{OUTPUT_DIR}CLIN_{USE_CLINICAL_FEATURES}_MRI_{USE_MRI_FEATURES}_RAD_{USE_RADIOMIC_FEATURES}_WSI_{USE_WSI_FEATURES}_MAG_{MAG}_PATCH_{PATCH_SIZE}_PATCHES_{PATCHES}_MASK_{MASKER}_EMBED_{EMBEDDER}_MODEL_{SURVIVAL_MODEL}_FUSE_{FUSION_TYPE}_loss_{DEEPHIT_LOSS}_SCALE_{SCALE_DATA}_EP_{EPOCHS}_ModDrop_{DROP_MODALITY}_binEdg_{BIN_EDGES}/" ## main path for results

GLOBAL_DIR = f"{OUTPUT_DIR}task1_submission5_clin_radiomic_wsi_500_patches"