...
SURVIVAL_WEIGHTS_PATH = Path("/opt/app/resources/clinv1_wsi")
SCALER_WEIGHTS_PATH = Path("/opt/app/resources/clinv1_wsi")
...
def get_or_fit_scaler(name, train_array, fit=True, fold=0):
    ...
    scaler_path = os.path.join(SCALER_WEIGHTS_PATH, f"{name}_scaler_run_1_{fold}.pkl")
...
def predict_score(clin_feats, mri_feats, wsi_feats):
    ...
    if USE_ENSEMBLE:
        ...
        for fold_idx in range(NUM_FOLDS):
            model_path = os.path.join(SURVIVAL_WEIGHTS_PATH, f"best_model_run1_fold{fold_idx}.pt")
