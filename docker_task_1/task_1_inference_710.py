...
from config710 import *
from model710 import MultimodalSurvivalModel
import h5py
...
def get_or_fit_scaler(name, train_array, fit=True, fold=0):
    ...
    scaler_path = os.path.join(SCALER_WEIGHTS_PATH, f"{name}_scaler_run_4_{fold}.pkl")
...
def predict_score(clinical_feats, radiomic_feats, wsi_feats):
    ...
    if USE_ENSEMBLE:
        ...
        for fold_idx in range(NUM_FOLDS):
            model_path = os.path.join(SURVIVAL_WEIGHTS_PATH, f"best_model_run4_fold{fold_idx}.pt")
